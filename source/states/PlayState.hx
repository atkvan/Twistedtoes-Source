...
class PlayState extends MusicBeatState
{
	...
	var folderDir:String = "assets/scripts/";
		...
		switch(eventName) {
			...
			case 'Flash Camera': 
					if (ClientPrefs.data.flashing) {
						...
						var camera:String = '';
						var color:String = '';
						if (split[0] != null) camera = split[0].trim();
						if (split[1] != null) color = split[1].trim();
				...
				LuaUtils.cameraFromString(camera).flash(CoolUtil.colorFromString(color), flValue1, null, true);
			}
			case 'Tween Camera':
				cameraTwnEvent = FlxTween.tween(FlxG.camera.zoom, {zoom: flValue1}, flValue2, {ease: FlxEase.quadInOut, onComplete: 
					function(twn:FlxTween) {
						FlxG.camera.zoom = flValue1;
				} 
			});
